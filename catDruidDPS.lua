...
function CatDruidDPS_main(mainDamage, opener, finisher, isPowerShift, druidBarAddon, isUseConsumables, isSelfInnervate)
	...
	if (druidBarAddon == "DruidBar") then
		...
		--print('JGP DruidBarKey.subtractmana:' .. tostring(DruidBarKey.subtractmana) .. ' /DruidBarKey.keepthemana:' .. tostring(DruidBarKey.keepthemana))
		if(shiftCost <= currentMana or runeOfMetamorphosis == true or (isSelfInnervate == true and innervateCooldown == 0) or (isUseConsumables == true
			and (CatDruidDPS_canUseConsumable("potion") ~= nil or CatDruidDPS_canUseConsumable("nightdragon") ~= nil or CatDruidDPS_canUseConsumable("rune") ~= nil))) then
				canPowerShift = true
		else
			canPowerShift = false
		end
	...
	--print('JGP 1, minenergy:' .. tostring(minEnergy))
	...
	--print('JGP currentForm:' .. tostring(currentForm) .. '/prowl:' .. tostring(prowl) .. '/energy>oe:' .. tostring(energy) .. '>' .. tostring(openerEnergy))
